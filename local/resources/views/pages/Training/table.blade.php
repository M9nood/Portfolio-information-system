          ...
          <div role="tabpanel" class="tab-pane active" id="home">
            <div class="">
              <div class="title-content" >
                <h5>รายการผลงานเข้ารับฝึกอบรมของ : {{Auth::user()->name." ".Auth::user()->lastname}}</h5>
              </div>
              <div class="pull-right">
                <a class="btn btn-primary btn-flat" data-toggle="tooltip" title="เพิ่ม" href="{{url('/training/add')}}"><i class="fa fa-lg fa-plus"></i></a><a class="btn btn-info btn-flat" data-toggle="tooltip" title="รีเฟรช" onClick="refresh()"><i class="fa fa-lg fa-refresh"></i></a>
              </div>
            </div>
            <div class="row">
              <div class="col-sm-12">
                    ...
                    <table class="table table-hover table-bordered " id="sampleTable" >
                      <thead>
                        <tr style="background-color:#8cb3d9">
                          ...
                          <th width="70">วันที่เข้ารับอบรม</th>
                          <th width="200">ชื่องาน</th>
                          <th width="200">สถานที่</th>
                          <th width="65"></th>
                        </tr>
                      </thead>
                      <tbody>
                          @foreach ($datas as $data)
                            <tr>
                              ...
                              <td style="text-align:center">
                                <a class="btn btn-info btn-xs" style="padding:5px"  title="รายละเอียด"  data-toggle="modal" data-target="#viewModal"  data-id="{{$data->trn_id}}"><i class="fa fa-info-circle"></i></a>
                                <a class="btn btn-warning btn-xs" style="padding:5px;"  title="แก้ไข"  href="{{url('/training/edit/'.$data->trn_id)}}" @if(Auth::user()->id != $data->personnel_id) disabled @endif><i class="fa fa-pencil-square-o"></i></a>
                                <a class="btn btn-danger btn-xs "  style="padding:5px;"  title="ลบ" data-toggle="modal" data-target="#confirmDeleteModal" data-name="{{$data->trn_name}}" data-id="{{$data->trn_id}}"  @if(Auth::user()->id != $data->personnel_id) disabled @endif><i class="fa fa-trash-o"></i></a>
                              </td>
                            </tr>
                          @endforeach
                      </tbody>
